...
from modules.gridnet.modules import LateralBlock, DownSamplingBlock, UpSamplingBlock
class GridNet(nn.Module):
    def __init__(self, out_chs=3, grid_chs=[32, 64, 96]):
        ...
        self.lateral_init = LateralBlock(70+64*2, self.n_chs[0])
        ...
        self.conv_3 = nn.Conv2d(in_channels=out_chs, out_channels=3, kernel_size=3, stride=1, padding=1)
    def forward(self, x_l1,x_l2,x_l3):
        ...
        out_img = self.lateral_final(state_05)
        
        return out_img


class Light_GridNet(nn.Module):
    def __init__(self, out_chs=3, grid_chs=[32, 64, 96]):
        # n_row = 3, n_col = 6, n_chs = [32, 64, 96]):
        super().__init__()
        ...
        for r, (in_ch, out_ch) in enumerate(zip(self.n_chs[1:], self.n_chs[:-1])):
            for c in range(int(self.n_col / 2-1)):
                setattr(self, f'up_{r}_{c}', UpSamplingBlock(in_ch, out_ch))
