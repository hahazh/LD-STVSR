...
from modules.pwc.utils.constv import ConstV
...
def backwarp(tenInput, tenFlow):
	...
	if str(tenFlow.shape) not in backwarp_tenPartial:
		backwarp_tenPartial[str(tenFlow.shape)] = tenFlow.new_ones([ tenFlow.shape[0], 1, tenFlow.shape[2], tenFlow.shape[3] ]).detach().cpu()
	# end
	
	tenFlow = torch.cat([ tenFlow[:, 0:1, :, :] / ((tenInput.shape[3] - 1.0) / 2.0), tenFlow[:, 1:2, :, :] / ((tenInput.shape[2] - 1.0) / 2.0) ], 1)
	if backwarp_tenPartial[str(tenFlow.shape)].device!=tenInput.device:
		
		backwarp_tenPartial[str(tenFlow.shape)] = backwarp_tenPartial[str(tenFlow.shape)].to(tenInput.device)
	...
	out = (tenOutput[:, :-1, :, :] * tenMask).to(tenInput.device)
	return out
# end
...
class Network(torch.nn.Module):
	def __init__(self):
		...
		# self.load_state_dict({ strKey.replace('module', 'net'): tenWeight for strKey, tenWeight in torch.load('./modules/pwc/weights/network-default.pytorch').items()})
	# end
